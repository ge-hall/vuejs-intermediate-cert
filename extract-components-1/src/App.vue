...
function updateRating(id, rating) {
  // find movie by id
  console.log(`updateRating ${id} - ${rating}`);
  findMovieById(id).rating = rating;
}

function removeMovie(id) {
  console.log(`movie to remove: ${id}`);
...
const averageRating = computed(() => {
  ...
  let averageRating =
    movies.value.length === 0
      ? 0
      : Number(avg / movies.value.length).toFixed(1);
  jki;
