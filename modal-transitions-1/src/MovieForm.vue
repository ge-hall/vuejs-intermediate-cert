...
function cancelForm() {
  ...
  cleanUpForm();
  console.log(`Form cleaned, emiting cancel.`);
  emit("cancel");
}
...
onMounted(() => {
  console.log(`modelValue = ${JSON.stringify(props.modelValue)}`);
});
watch(props.modelValue, (newVal, oldVal) => {
  console.log(
    `modelValue changed from ${JSON.stringify(oldVal)} to ${JSON.stringify(newVal)}`,
  );
});
</script>
